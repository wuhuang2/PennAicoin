...
from PySide6.QtWidgets import (
    ...
    QTabBar, QToolButton, QSpacerItem, QMenu
)
from PySide6.QtCore import QDate, Qt, QTimer, Signal, QThread, QSettings, QSize, QUrl
from PySide6.QtGui import QIcon, QPixmap, QKeySequence, QFont, QPalette, QColor, QLinearGradient, QBrush, QPainter, QCursor, QShortcut, QAction
...
class FileManager:
    ...
    def read_encrypted_file(self, password, encrypted_data):
        try:
            # 从文件中加载私钥
...
class SettingsDialog(ThemedDialog):
    def __init__(self, parent=None):
        ...
        self.menu_list.setCurrentRow(1)  # 默认选中“关于”
...
class AboutDialog(ThemedDialog):
    def __init__(self, parent=None):
        ...
        # 软件名称和版本
        name_label = QLabel("PennAicoin锦云策")
        name_label.setStyleSheet("font-size: 18px; font-weight: bold; margin: 10px 0;")
        name_label.setAlignment(Qt.AlignCenter)
        
        version_label = QLabel("版本:PennAicoin_V0.1.1.2025.07.25_01_alpha ")
...
class PennAicoinMainWindow(QMainWindow):
    def __init__(self):
        ...
        self.shortcuts = {}
        self.file_manager = FileManager()  # 初始化文件管理器
        ...
        try:
            self.setWindowIcon(QIcon(resource_path('resources/app.png')))
        except:
            pass  # 图标加载失败时忽略
    ...
    def apply_styles(self):
        ...
        self.setStyleSheet("""
            ...
            #newTabBtn:hover, #newTabBtn:pressed {
    ...
    def create_left_navigation(self):
        ...
        try:
            pixmap = QPixmap(resource_path("resources/app.png"))
            if not pixmap.isNull():
                app_icon.setPixmap(pixmap.scaled(32, 32, Qt.KeepAspectRatio, Qt.SmoothTransformation))
            else:
                app_icon.setText("图标")  # 图片加载失败时显示文本
        except:
            app_icon.setText("图标")  # 发生异常时显示文本
        ...
        try:
            icon = QIcon(resource_path("resources/home.ico"))
        ...
        try:
            icon = QIcon(resource_path("resources/voice.ico"))
        ...
        try:
            icon = QIcon(resource_path("resources/settings.ico"))
        ...
        try:
            icon = QIcon(resource_path("resources/info.ico"))
    ...
    def create_top_navigation(self):
        ...
        # 导入导出按钮 - CSV类
        self.csv_btn = QPushButton("CSV类")
        self.csv_btn.setProperty("class", "importExportBtn")
        self.csv_btn.setMenu(self.create_csv_menu())
        top_layout.addWidget(self.csv_btn)
        
        # 导入导出按钮 - JZRJ类
        self.jzrj_btn = QPushButton("JZRJ类")
        self.jzrj_btn.setProperty("class", "importExportBtn")
        self.jzrj_btn.setMenu(self.create_jzrj_menu())
        top_layout.addWidget(self.jzrj_btn)
    ...
    def create_csv_menu(self):
        """创建CSV类的导入导出菜单"""
        menu = QMenu(self)
        export_action = menu.addAction("导出CSV文件")
        export_action.triggered.connect(self.export_to_csv)
        
        import_action = menu.addAction("导入CSV文件")
        import_action.triggered.connect(self.import_from_csv)
        
        return menu
        
    def create_jzrj_menu(self):
        """创建JZRJ类的导入导出菜单"""
        menu = QMenu(self)
        export_action = menu.addAction("加密导出JZRJ文件")
        export_action.triggered.connect(self.export_to_jzrj)
        
        import_action = menu.addAction("解密导入JZRJ文件")
        import_action.triggered.connect(self.import_from_jzrj)
    ...
    def show_export_menu(self):
        """显示导出菜单（快捷键触发）"""
        # 显示CSV导出菜单作为默认
        pos = self.csv_btn.mapToGlobal(self.csv_btn.rect().bottomLeft())
        self.csv_btn.menu().exec(pos)
    ...
    def delete_selected_records(self):
        ...
        if reply == QMessageBox.Yes:
            try:
                ...
                print(f"删除记录时出错: {e}")
                QMessageBox.warning(self, "错误", f"删除记录时出错: {str(e)}")
    ...
    def update_time(self):
        ...
        self.app_name.setText(f"PennAicoin锦云策  |  {current_time}")
    ...
    def add_new_tab(self, title, file_type=None, file_path=None):
        ...
        if title == "欢迎使用":
            welcome_label = QLabel("欢迎使用PennAicoin锦云策\n\n请使用左侧导航栏访问不同功能，或打开一个文件开始工作。")
            ...
            new_table.setColumnCount(6)
            new_table.setHorizontalHeaderLabels(["日期", "金额", "币种", "收支类型", "详细分类", "备注信息"])
    ...
    def create_new_tab(self):
        ...
        self.add_new_tab(f"新账本{tab_count}")
    ...
    def export_to_jzrj(self):
        ...
        try:
            ...
            if file_dialog.exec():
                file_path = file_dialog.selectedFiles()[0]
                # 确保文件扩展名正确
                if not file_path.endswith('.jzrj'):
                    file_path += '.jzrj'
                ...
                if ok and password:
                    ...
                    if self.file_manager.export_to_jzrj("accounting.db", base_name, password):
                        QMessageBox.information(
                            self, "导出成功", 
                            f"数据已成功加密导出到: {file_path}\n"
                            f"请妥善保管您的私钥文件(private_key.pem)和密码！"
                        )
                    else:
                        QMessageBox.warning(self, "导出失败", "加密导出数据时发生错误！")
                    
        except Exception as e:
            print(f"加密导出数据时出错: {e}")
            QMessageBox.critical(self, "错误", f"加密导出数据时出错: {str(e)}")
            
    def import_from_jzrj(self):
        """从.jzrj文件解密导入数据"""
        try:
            file_dialog = QFileDialog()
            file_dialog.setWindowTitle("从JZRJ文件导入")
            file_dialog.setLabelText(QFileDialog.Accept, "打开")
            file_dialog.setNameFilter("JZRJ文件 (*.jzrj)")
            file_dialog.setFileMode(QFileDialog.ExistingFile)
            ...
            if file_dialog.exec():
                ...
                if ok and password:
                    if self.file_manager.import_from_jzrj(file_path, password):
                        QMessageBox.information(
                            self, "导入成功", 
                            f"数据已成功从: {file_path} 解密导入"
                        )
                        self.load_records()  # 刷新记录
                    else:
                        QMessageBox.warning(self, "导入失败", "解密导入数据时发生错误！")
                    
        except Exception as e:
            print(f"解密导入数据时出错: {e}")
            QMessageBox.critical(self, "错误", f"解密导入数据时出错: {str(e)}")
