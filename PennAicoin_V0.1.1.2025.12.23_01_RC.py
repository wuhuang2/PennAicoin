...
from PySide6.QtWidgets import (
    ...
    QDialog, QHeaderView, QTextEdit, QListWidget, 
    QListWidgetItem, QStackedWidget, QFileDialog, QTabWidget, QInputDialog, QGroupBox, QKeySequenceEdit, QFrame, QScrollArea
)
from PySide6.QtCore import QDate, Qt, QTimer, Signal, QThread, QSize
from PySide6.QtGui import QIcon, QPixmap, QKeySequence, QFont, QShortcut
...
class SettingsDialog(ThemedDialog):
    def __init__(self, parent=None):
        ...
        menu_items = ["用户协议", "文件管理", "文件加密", "快捷键", "关于"]
        for item in menu_items:
            list_item = QListWidgetItem(item)
            list_item.setTextAlignment(Qt.AlignCenter)
            self.menu_list.addItem(list_item)
            
        self.menu_list.setCurrentRow(0)  # 默认选中“用户协议”
    ...
    def create_user_agreement_page(self):
        ...
        scroll_layout.addWidget(self.agreement_text)
    ...
    def create_file_encryption_page(self):
        ...
        # 创建加密功能组
        encryption_group = QGroupBox("数据加密/解密")
    ...
    def create_shortcuts_page(self):
        ...
        # 快捷键设置表单
        shortcuts_form = QGroupBox("快捷键配置")
        shortcuts_form.setStyleSheet("""
            QGroupBox {
                font-size: 16px;
                font-weight: bold;
                padding: 10px;
        ...
        """)
        form_layout = QVBoxLayout(shortcuts_form)
        ...
        for action, default_shortcut in shortcut_pairs:
            ...
            shortcut_edit.setKeySequence(QKeySequence(default_shortcut))
            shortcut_edit.setFixedWidth(200)
            shortcut_edit.setStyleSheet("""
                QKeySequenceEdit {
                    padding: 8px;
            ...
            h_layout.addWidget(shortcut_edit)
            h_layout.addStretch()
        ...
        scroll_layout.addWidget(shortcuts_form)
    ...
    def create_about_page(self):
        ...
        app_name_label = QLabel("PennAicoin 锦云策")
        ...
        info_text.addWidget(app_name_label)
        info_text.addWidget(version_label)
        info_text.addWidget(dev_label)
        info_text.addWidget(copyright_label)
        info_text.addStretch()
        
        app_info_layout.addLayout(info_text)
        app_info_layout.addStretch()
        
        card_layout.addLayout(app_info_layout)
        ...
        self.mit_license_zh_cn_label.setStyleSheet("""
            QTextEdit {
                background-color: #f8f9fa;
                border: 1px solid #e9ecef;
                border-radius: 6px;
        ...
        license_layout.addWidget(self.mit_license_zh_cn_label)
...
class PennAicoinMainWindow(QMainWindow):
    ...
    def init_shortcuts(self):
        ...
        QShortcut(self.shortcuts["about"], self).activated.connect(self.show_about_dialog)
        QShortcut(self.shortcuts["voice"], self).activated.connect(self.show_record_dialog)
        QShortcut(self.shortcuts["search"], self).activated.connect(lambda: self.search_box.setFocus())
    ...
    def create_top_navigation(self):
        ...
        self.user_avatar.setStyleSheet("border-radius: 18px; background-color: #e1e4e8;")
        self.user_avatar.setIcon(QIcon(resource_path("resources/user.ico")))
        self.user_avatar.setIconSize(QSize(36, 36))
        top_layout.addWidget(self.user_avatar)
    ...
    def load_records(self):
        ...
        try:
            ...
            for row, record in enumerate(records):
                # 确保记录有足够的字段
                if len(record) >= 7:
                    date_item = QTableWidgetItem(str(record[1]) if record[1] is not None else "")
                    date_item.setData(Qt.UserRole, record[0])  # 存储ID
                    self.table_widget.setItem(row, 0, date_item)
                    self.table_widget.setItem(row, 1, QTableWidgetItem(str(record[2]) if record[2] is not None else ""))
                    self.table_widget.setItem(row, 2, QTableWidgetItem(str(record[3]) if record[3] is not None else ""))
                    self.table_widget.setItem(row, 3, QTableWidgetItem(str(record[4]) if record[4] is not None else ""))
                    self.table_widget.setItem(row, 4, QTableWidgetItem(str(record[5]) if record[5] is not None else ""))
                    self.table_widget.setItem(row, 5, QTableWidgetItem(str(record[6]) if record[6] is not None else ""))
        except Exception as e:
            print(f"加载记录时出错: {e}")
