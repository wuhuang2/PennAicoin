...
from PySide6.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, QHBoxLayout, 
                             ...
                             QMessageBox, QSplitter, QLineEdit, QFrame, QSizePolicy)
from PySide6.QtCore import Qt, Signal, QObject, QThread, QSize
from PySide6.QtGui import QFont, QTextCursor, QColor, QIcon, QPalette, QBrush, QPixmap
    ...
    def run(self):
        try:
            ...
            for token in self.model.create_completion(
                ...
                self.signals.result_ready.emit(token_text)
                progress += 1
...
class AIChatWindow(QMainWindow):
    ...
    def init_ui(self):
        # 窗口基本设置
        self.setWindowTitle("PennAicoin AI助手")
        ...
        title_frame.setStyleSheet("background-color: #f0f2f5; border-radius: 5px;")
        title_layout = QHBoxLayout(title_frame)
        title_label = QLabel("PennAicoin AI 智能助手")
        title_label.setFont(QFont("SimHei", 14, QFont.Bold))
        title_label.setStyleSheet("color: #1a73e8;")
        title_layout.addWidget(title_label, alignment=Qt.AlignLeft | Qt.AlignVCenter)
        main_layout.addWidget(title_frame)
        
        # 控制区 - 优化布局和样式
        control_frame = QFrame()
        control_frame.setFrameShape(QFrame.StyledPanel)
        control_frame.setStyleSheet("background-color: #f8f9fa; border-radius: 5px;")
        control_layout = QVBoxLayout(control_frame)
        
        # 模型控制行
        model_control_layout = QHBoxLayout()
        model_control_layout.setSpacing(8)
        
        self.model_path_edit = QLineEdit()
        self.model_path_edit.setPlaceholderText("模型文件路径 (GGUF格式)")
        self.model_path_edit.setReadOnly(True)
        self.model_path_edit.setStyleSheet("padding: 5px; border: 1px solid #ddd; border-radius: 3px;")
        model_control_layout.addWidget(self.model_path_edit, 3)
        ...
        model_control_layout.addWidget(self.select_model_btn)
        ...
        file_control_layout.setSpacing(8)
        
        self.file_path_edit = QLineEdit()
        self.file_path_edit.setPlaceholderText("文件路径 (支持CSV, DOCX, TXT)")
        self.file_path_edit.setReadOnly(True)
        self.file_path_edit.setStyleSheet("padding: 5px; border: 1px solid #ddd; border-radius: 3px;")
        file_control_layout.addWidget(self.file_path_edit, 3)
        ...
        file_control_layout.addWidget(self.upload_file_btn)
        
        control_layout.addLayout(model_control_layout)
        control_layout.addLayout(file_control_layout)
        main_layout.addWidget(control_frame)
        
        # 聊天区域 - 带边框和样式
        chat_frame = QFrame()
        chat_frame.setFrameShape(QFrame.StyledPanel)
        chat_frame.setStyleSheet("border: 1px solid #ddd; border-radius: 5px;")
        ...
        self.send_btn.setIcon(QIcon.fromTheme("mail-send", QIcon()))
        ...
        status_frame.setStyleSheet("background-color: #f8f9fa; border-radius: 5px;")
